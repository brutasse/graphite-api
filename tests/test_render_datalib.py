...
class TimeSeriesTest(TestCase):
    ...
    def test_TimeSeries_init_string_values(self):
        ...
        expected = TimeSeries(
            "collectd.test-db.load.value", 0, 2, 1, ["a", "b"]
        )
    ...
    def test_TimeSeries_equal_list(self):
        values = range(0, 100)
        series = TimeSeries(
            "collectd.test-db.load.value", 0, len(values), 1, values
        )
    ...
    def test_TimeSeries_consolidate(self):
        values = range(0, 100)
        series = TimeSeries(
            "collectd.test-db.load.value", 0, len(values) / 2, 1, values
        )
    ...
    def test_TimeSeries_iterate_valuesPerPoint_2_none_values(self):
        ...
        self.assertEqual(series.valuesPerPoint, 2)
        expected = TimeSeries(
            "collectd.test-db.load.value", 0, 5, 1, [None, None, None]
        )
    ...
    def test_TimeSeries_iterate_valuesPerPoint_2_avg(self):
        ...
        expected = TimeSeries(
            ...
            5,
            1,
            list(map(lambda x: x + 0.5, range(0, 100, 2))) + [None],
        )
        self.assertEqual(list(series), list(expected))
    ...
    def test_TimeSeries_iterate_valuesPerPoint_2_sum(self):
        values = range(0, 100)
        series = TimeSeries(
            "collectd.test-db.load.value", 0, 5, 1, values, consolidate="sum"
        )
        self.assertEqual(series.valuesPerPoint, 1)
        series.consolidate(2)
        self.assertEqual(series.valuesPerPoint, 2)
    ...
    def test_TimeSeries_iterate_valuesPerPoint_2_max(self):
        values = range(0, 100)
        series = TimeSeries(
            "collectd.test-db.load.value", 0, 5, 1, values, consolidate="max"
        )
        self.assertEqual(series.valuesPerPoint, 1)
        series.consolidate(2)
        self.assertEqual(series.valuesPerPoint, 2)
    ...
    def test_TimeSeries_iterate_valuesPerPoint_2_min(self):
        values = range(0, 100)
        series = TimeSeries(
            "collectd.test-db.load.value", 0, 5, 1, values, consolidate="min"
        )
        self.assertEqual(series.valuesPerPoint, 1)
        series.consolidate(2)
        self.assertEqual(series.valuesPerPoint, 2)
...
class DatalibFunctionTest(TestCase):
    ...
    def test_nonempty_false_nones(self):
        series = TimeSeries(
            "collectd.test-db.load.value", 0, 4, 1, [None, None, None, None]
        )
        self.assertFalse(nonempty(series))
