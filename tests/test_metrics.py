...
class MetricsTests(TestCase):
    ...
    def test_find(self):
        ...
        response = self.app.get(
            url, query_string={"query": "test", "format": "completer"}
        )
        self.assertJSON(response, {"metrics": []})

        response = self.app.get(
            url, query_string={"query": "test", "format": "nodelist"}
        )
        ...
        for _url in ["/metrics/find", "/metrics"]:
            response = self.app.get(
                _url, query_string={"query": "test.*", "format": "treejson"}
            )
            self.assertJSON(
                response,
                [
        ...
        self.assertJSON(
            ...
            {
                "metrics": [
                    {"is_leaf": 0, "name": "bar", "path": "test.bar."},
                    {"is_leaf": 1, "name": "foo", "path": "test.foo"},
                    {"is_leaf": 0, "name": "wat", "path": "test.wat."},
                ]
            },
        )
        ...
        response = self.app.get(
            url, query_string={"query": "test.*", "format": "json"}
        )
        ...
        self.assertTrue(
            int(data[1]["intervals"][0]["start"]) in [ts - 60, ts - 59]
        )
        ...
        response = self.app.get(
            url, query_string={"query": "*", "jsonp": "foo", "format": "json"}
        )
        data = response.data.decode("utf-8")
        self.assertEqual(
            json.loads(data.split("(")[1].strip(")")),
            [{"is_leaf": False, "intervals": [], "path": "test"}],
        )

        response = self.app.get(
            url, query_string={"query": "*", "format": "nodelist"}
        )
    ...
    def test_find_validation(self):
        url = "/metrics/find"
        response = self.app.get(
            url, query_string={"query": "foo", "wildcards": "aaa"}
        )
        self.assertJSON(
            response,
        ...
        response = self.app.get(
            url, query_string={"query": "foo", "from": "aaa", "until": "bbb"}
        )
        self.assertJSON(
            response,
            {
        ...
        response = self.app.get(
            url, query_string={"query": "foo", "format": "other"}
        )
        self.assertJSON(
            response,
            {"errors": {"format": 'unrecognized format: "other".'}},
            status_code=400,
        )
    ...
    def test_expand(self):
        ...
        response = self.app.get(url, query_string={"query": "test.*"})
        self.assertJSON(
            response, {"results": ["test.bar", "test.foo", "test.wat"]}
        )

        response = self.app.get(
            url, query_string={"query": "test.*", "leavesOnly": 1}
        )
        self.assertJSON(response, {"results": ["test.foo"]})

        response = self.app.get(
            url, query_string={"query": "test.*", "groupByExpr": 1}
        )
        self.assertJSON(
            response,
            {"results": {"test.*": ["test.bar", "test.foo", "test.wat"]}},
        )
    ...
    def test_noop(self):
        ...
        self.assertJSON(
            response,
            {"error": "Dashboard 'foo' does not exist."},
            status_code=404,
        )
    ...
    def test_metrics_index(self):
        ...
        response = self.app.get(url)
        self.assertJSON(
            response, [u"collectd.cpu", u"collectd.load", u"collectd.memory"]
        )
