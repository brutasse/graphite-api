...
class RenderTest(TestCase):
    ...
    def test_render_view(self):
        ...
        response = self.app.get(
            self.url, query_string={"target": "test", "format": "pdf"}
        )
        self.assertEqual(
        ...
        self.create_db()
        response = self.app.get(
            self.url, query_string={"target": "test", "format": "json"}
        )
        ...
        self.assertEqual(
            data[0]["datapoints"],
            [[1.0, self.ts - 2], [0.5, self.ts - 1], [1.5, self.ts]],
        )

        response = self.app.get(
            self.url, query_string={"target": "test", "format": "raw"}
        )
        try:
            self.assertEqual(
                response.data.decode("utf-8"),
                "test,%d,%d,1|%s"
                % (self.ts - 59, self.ts + 1, "None," * 57 + "1.0,0.5,1.5\n"),
            )
        except AssertionError:
            self.assertEqual(
                response.data.decode("utf-8"),
        ...
        response = self.app.get(
            self.url, query_string={"target": "test", "format": "dygraph"}
        )
        data = json.loads(response.data.decode("utf-8"))
        end = data["data"][-4:]
        try:
        ...
        response = self.app.get(
            self.url, query_string={"target": "test", "format": "rickshaw"}
        )
        data = json.loads(response.data.decode("utf-8"))
        end = data[0]["datapoints"][-4:]
        try:
    ...
    def test_render_constant_line(self):
        response = self.app.get(
            self.url, query_string={"target": "constantLine(12)"}
        )
        self.assertEqual(response.headers["Content-Type"], "image/png")

        response = self.app.get(
            self.url,
            query_string={"target": "constantLine(12)", "format": "json"},
        )
    ...
    def test_sumseries(self):
        ...
        for index, value in enumerate(agg["baz"]):
            self.assertEqual(
                value, agg["sumSeries(sin(bar),sin(foo))"][index]
            )
    ...
    def test_render_validation(self):
        ...
        response = self.app.get(
            self.url, query_string={"graphType": "foo", "target": "test"}
        )
        self.assertJSON(
            response,
            {
                "errors": {
                    "graphType": "Invalid graphType 'foo', must be one of "
                    "'line', 'pie'."
                }
            },
            status_code=400,
        ...
        response = self.app.get(
            self.url, query_string={"maxDataPoints": "foo", "target": "test"}
        )
        self.assertJSON(
            response,
        ...
        response = self.app.get(
            self.url, query_string={"target": "foo", "tz": "Europe/Lausanne"}
        )
        self.assertJSON(
            response,
            {"errors": {"tz": "Unknown timezone: 'Europe/Lausanne'."}},
            status_code=400,
        )

        response = self.app.get(
            self.url, query_string={"target": "test:aa", "graphType": "pie"}
        )
        self.assertJSON(
            response,
            {"errors": {"target": "Invalid target: 'test:aa'."}},
            status_code=400,
        )

        response = self.app.get(
            self.url,
            query_string={"target": ["test", "foo:1.2"], "graphType": "pie"},
        )
        self.assertEqual(response.status_code, 200)

        response = self.app.get(
            self.url, query_string={"target": ["test", ""]}
        )
        self.assertEqual(response.status_code, 200)

        response = self.app.get(
            self.url, query_string={"target": "test", "format": "csv"}
        )
        ...
        response = self.app.get(
            self.url,
            query_string={"target": "test", "format": "svg", "jsonp": "foo"},
        )
        ...
        response = self.app.get(
            self.url, query_string={"target": "test", "format": "svg"}
        )
        svg = response.data.decode("utf-8")
        self.assertTrue(svg.startswith('<?xml version="1.0"'))

        response = self.app.get(
            self.url, query_string={"target": "inexisting", "format": "svg"}
        )
        self.assertEqual(response.status_code, 200)
        svg = response.data.decode("utf-8")
        self.assertTrue(svg.startswith('<?xml version="1.0"'))
        ...
        response = self.app.get(
            self.url, query_string={"target": "sum(test)"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_raw_data(self):
        ...
        response = self.app.get(
            self.url, query_string={"rawData": "1", "target": "test"}
        )
    ...
    def test_jsonp(self):
        ...
        response = self.app.get(
            self.url,
            query_string={"format": "json", "jsonp": "foo", "target": "test"},
        )
    ...
    def test_templates(self):
        ...
        for query, expected in [
            ...
            (
                {"target": "template(constantLine($1))", "template[1]": "12"},
                "12.0",
            ),
