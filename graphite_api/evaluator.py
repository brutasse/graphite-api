...
def pathsFromTokens(requestContext, tokens, replacements=None):
    ...
    if tokens.template:
        ...
        iters.append(
            pathsFromTokens(requestContext, tokens.template, arglist)
        )

    elif tokens.expression:
        iters.append(
            pathsFromTokens(requestContext, tokens.expression, replacements)
        )
        ...
        if tokens.call.funcname == "template":
            ...
            raise ValueError(
                "invalid template() syntax, only string/numeric "
                "arguments are allowed"
            )

        iters.extend(
            [
                pathsFromTokens(requestContext, arg, replacements)
                for arg in tokens.call.args
            ]
        )
        iters.extend(
            [
                pathsFromTokens(requestContext, kwarg.args[0], replacements)
...
def evaluateTokens(
    requestContext, tokens, data_store=None, replacements=None
):
    if data_store is None:
        paths = list(pathsFromTokens(requestContext, tokens))
        data_store = fetchData(requestContext, paths)
    ...
    if tokens.template:
        ...
        if tokens.template.kwargs:
            args = [
                (
                    kwarg.argname,
                    evaluateTokens(requestContext, kwarg.args[0], data_store),
                )
                for kwarg in tokens.template.kwargs
            ]
        ...
        if tokens.template.args:
            args = [
                (str(i + 1), evaluateTokens(requestContext, arg, data_store))
                for i, arg in enumerate(tokens.template.args)
            ]
            arglist.update(dict(args))
        if "template" in requestContext:
        ...
        return evaluateTokens(
            requestContext, tokens.template, data_store, arglist
        )

    elif tokens.expression:
        return evaluateTokens(
            requestContext, tokens.expression, data_store, replacements
        )
        ...
        args = [
            evaluateTokens(requestContext, arg, data_store, replacements)
            for arg in tokens.call.args
        ]
