...
def dashboard_load(name):
    return jsonify(
        {"error": "Dashboard '{0}' does not exist.".format(name)}, status=404
    )
...
def render():
    ...
    errors = {}
    graph_options = {"width": 600, "height": 300}
    request_options = {}
    ...
    if use_cache:
        ...
        if response is not None:
            logger.debug(
                "cached response", time=(time.time() - start), targets=targets
            )
            return response
    ...
    headers = (
        ...
        if use_cache
        else {"Pragma": "no-cache", "Cache-Control": "no-cache"}
    )
    ...
    if request_options["graphType"] == "pie":
        for target in request_options["targets"]:
            if ":" in target:
                ...
                for series in series_list:
                    ...
                    context["data"].append(
                        (series.name, func(context, series) or 0)
                    )
        ...
        if request_options["format"] == "csv":
            ...
            for series in context["data"]:
                for index, value in enumerate(series):
                    ...
                    writer.writerow(
                        (series.name, ts.strftime("%Y-%m-%d %H:%M:%S"), value)
                    )
            response.seek(0)
        ...
        if request_options["format"] == "json":
            ...
            if "maxDataPoints" in request_options and any(context["data"]):
                ...
                for series in context["data"]:
                    ...
                    if len(values) > 0:
                        series_data.append(
                            {"target": series.name, "datapoints": values}
                        )
            else:
                for series in context["data"]:
                    timestamps = range(
                        series.start, series.end + series.step, series.step
                    )
                    datapoints = zip(series, timestamps)
                    series_data.append(
            ...
            if use_cache:
                app.cache.add(request_key, response, cache_timeout)
            logger.debug(
                "rendered json", time=(time.time() - start), targets=targets
            )
            return response
        ...
        if request_options["format"] == "rickshaw":
            ...
            for series in context["data"]:
                ...
                datapoints = [
                    {"x": x, "y": y} for x, y in zip(timestamps, series)
                ]
                series_data.append(
    ...
    if use_svg and "jsonp" in request_options:
        ...
        response = (
            "{0}({1})".format(
                request_options["jsonp"], json.dumps(image.decode("utf-8"))
            ),
            200,
            headers,
        )
    ...
    if use_cache:
        app.cache.add(request_key, response, cache_timeout)
    logger.debug(
        "rendered graph", time=(time.time() - start), targets=targets
    )
    return response
...
def json_nodes(nodes):
    ...
    for node in nodes:
        info = {"path": node.path, "is_leaf": node.is_leaf, "intervals": []}
        if node.is_leaf:
