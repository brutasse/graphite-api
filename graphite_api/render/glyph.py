...
defaultGraphOptions = dict(
    ...
    minorline="grey",
    linecolors=(
        "blue,green,red,purple,brown,yellow,aqua,grey,"
...
class _AxisTics:
    ...
    def reconcileLimits(self):
        ...
        if minFixed and maxFixed:
            ...
            delta = self.chooseDelta(
                max(abs(self.minValue), abs(self.maxValue))
            )
    ...
    def makeLabel(self, value):
        ...
        span, spanPrefix = format_units(
            self.span, self.step, system=self.unitSystem
        )
        if prefix:
            prefix += " "
        if value < 0.1:
...
class _LinearAxisTics(_AxisTics):
    ...
    def chooseStep(self, divisors=None, binary=False):
        ...
        if divisors is None:
            ...
            for divisor in divisors:
                ...
                if divisor < 1:
                    raise GraphError(
                        "Divisors must be greater than or equal " "to one"
                    )
        ...
        for step in self.generateSteps(variance / float(max(divisors))):
            if (
                bestSlop is not None
                and step * min(divisors) >= 2 * bestSlop + variance
            ):
                break
            for divisor in divisors:
    ...
    def chooseLimits(self):
        ...
        if self.maxValueSource == "data":
            ...
            if (
                self.axisLimit is not None
                and self.top > self.axisLimit + EPSILON * self.step
            ):
...
class _LogAxisTics(_AxisTics):
    ...
    def chooseLimits(self):
        if self.minValue <= 0:
            raise GraphError(
                "Logarithmic scale specified with a dataset with "
                "a minimum value less than or equal to zero"
            )
        self.bottom = math.pow(
            self.base, math.floor(math.log(self.minValue, self.base))
        )
        self.top = math.pow(
            self.base, math.ceil(math.log(self.maxValue, self.base))
        )
    ...
    def getLabelValues(self):
        ...
        value = math.pow(
            self.base, math.ceil(math.log(self.bottom, self.base) - EPSILON)
        )
...
class Graph(object):
    ...
    def setColor(self, value, alpha=1.0, forceAlpha=False):
        if isinstance(value, tuple) and len(value) == 3:
            ...
            raise ValueError(
                "Must specify an RGB 3-tuple, an html color "
    ...
    def drawText(self, text, x, y, align="left", valign="top", rotate=0):
        ...
        self.ctx.rel_move_to(
            math.sin(angle) * -vertical, math.cos(angle) * vertical
        )
    ...
    def drawLegend(self, elements, unique=False):
        ...
        if self.secondYAxis and rightSideLabels:
            ...
            columns = max(
                1, math.floor((self.width - self.area["xmin"]) / labelWidth)
            )
            numRight = len(
                [name for (name, color, rightSide) in elements if rightSide]
            )
            numberOfLines = max(len(elements) - numRight, numRight)
            columns = math.floor(columns / 2.0)
            columns = max(columns, 1)
            legendHeight = (
                max(1, (numberOfLines / columns)) * lineHeight
            ) + padding
            ...
            for name, color, rightSide in elements:
                ...
                if rightSide:
                    ...
                    self.drawText(
                        name, xRight - boxSize, yRight, align="right"
                    )
                    xRight -= labelWidth
                    if nRight % columns == 0:
                        xRight = self.area["xmax"] - self.area["xmin"]
                    ...
                    self.setColor(self.foregroundColor)
                    self.drawText(
                        name, x + boxSize + padding, y, align="left"
                    )
                    x += labelWidth
                    if n % columns == 0:
                        x = self.area["xmin"]
    ...
    def loadTemplate(self, template):
        ...
        self.defaultBackground = opts.get(
            "background", defaults["background"]
        )
        self.defaultForeground = opts.get(
        ...
        self.defaultColorList = [
            c.strip()
            for c in opts.get("linecolors", defaults["linecolors"]).split(",")
        ]
        fontName = opts.get("fontname", defaults["fontname"])
        fontSize = float(opts.get("fontsize", defaults["fontsize"]))
        fontBold = (
            opts.get("fontbold", defaults["fontbold"]).lower() == "true"
        )
        fontItalic = (
            opts.get("fontitalic", defaults["fontitalic"]).lower() == "true"
        )
    ...
    def output(self, fileObj):
        if self.outputFormat == "png":
            ...
            if hasattr(self, "startTime"):
                has_data = True
                metaData = {
                    "x": {"start": self.startTime, "end": self.endTime},
...
class LineGraph(Graph):
    ...
    def drawGraph(self, **params):
        ...
        if self.secondYAxis and params.get("vtitleRight"):
            self.drawVTitle(
                force_text(params["vtitleRight"]), rightAlign=True
            )
        self.setFont()
        ...
        if not params.get("hideLegend", len(self.data) > 10):
            ...
            for series in self.data:
                if series.name:
                    if not (
                        hideNull and all(v is None for v in list(series))
                    ):
                        elements.append(
                            (
    ...
    def drawVTitle(self, text, rightAlign=False):
        ...
        if rightAlign:
            ...
            for line in text.split("\n"):
                self.drawText(
                    line, x, y, align="center", valign="baseline", rotate=90
                )
                x -= lineHeight
            ...
            for line in text.split("\n"):
                self.drawText(
                    line, x, y, align="center", valign="baseline", rotate=270
                )
                x += lineHeight
    ...
    def getYCoord(self, value, side=None):
        ...
        if self.logBase:
            if value <= 0:
                return None
            relativeValue = math.log(value, self.logBase) - math.log(
    ...
    def drawLines(
        self, width=None, dash=None, linecap="butt", linejoin="miter"
    ):
        if not width:
        ...
        for series in self.data:
            ...
            startShift = series.xStep * (
                missingPoints / series.valuesPerPoint
            )
            ...
            if series.options.get("invisible"):
                ...
                self.setColor(
                    series.color, series.options.get("alpha") or 1.0
                )
            ...
            for index, value in enumerate(series):
                ...
                if value is None:
                    ...
                    if self.lineMode == "staircase":
                        ...
                        if (
                            consecutiveNones > self.connectedLimit
                            or consecutiveNones == index
                        ):
                            self.ctx.move_to(x, y)
    ...
    def setupYAxis(self):
        ...
        (yMinValue, yMaxValue) = dataLimits(
            self.data, drawNullAsZero, stacked
        )
        if self.logBase:
            yTics = _LogAxisTics(
                yMinValue,
        ...
        if not self.params.get("hideAxes", False):
            ...
            self.yLabels = [
                yTics.makeLabel(value) for value in self.yLabelValues
            ]
    ...
    def setupTwoYAxes(self):
        ...
        (yMinValueL, yMaxValueL) = dataLimits(
            self.dataLeft, drawNullAsZero, stacked
        )
        (yMinValueR, yMaxValueR) = dataLimits(
            self.dataRight, drawNullAsZero, stacked
        )
        ...
        if self.logBase:
            ...
            yTicsL = _LinearAxisTics(
                yMinValueL,
                yMaxValueL,
                unitSystem=self.params.get("yUnitSystem"),
            )
            yTicsR = _LinearAxisTics(
                yMinValueR,
                yMaxValueR,
                unitSystem=self.params.get("yUnitSystem"),
            )

        yTicsL.applySettings(
            axisMin=self.params.get("yMinLeft"),
            axisMax=self.params.get("yMaxLeft"),
            axisLimit=self.params.get("yLimitLeft"),
        )
        yTicsR.applySettings(
            axisMin=self.params.get("yMinRight"),
        ...
        self.yLabelsL = [
            yTicsL.makeLabel(value) for value in self.yLabelValuesL
        ]
        self.yLabelsR = [
            yTicsR.makeLabel(value) for value in self.yLabelValuesR
        ]
    ...
    def setupXAxis(self):
        ...
        potential = [
            c
            for c in xAxisConfigs
            if c["seconds"] <= secondsPerPixel
        ...
        if not self.params.get("hideYAxis"):
            if not self.secondYAxis:
                for value, label in zip(self.yLabelValues, self.yLabels):
                    ...
                    if self.params.get("yAxisSide") == "left":
                        self.drawText(
                            label, x, y, align="right", valign="middle"
                        )
                    else:
                        # Inverted for right side Y Axis
                        self.drawText(
                            label, x, y, align="left", valign="middle"
                        )
                ...
                for valueL, labelL in zip(self.yLabelValuesL, self.yLabelsL):
                    ...
                    if yL is None:
                        ...
                        yL = 0
                    self.drawText(
                        labelL, xL, yL, align="right", valign="middle"
                    )
                ...
                for valueR, labelR in zip(self.yLabelValuesR, self.yLabelsR):
                    ...
                    # Inverted for right side Y Axis
                    self.drawText(
                        labelR, xR, yR, align="left", valign="middle"
                    )

        if not self.params.get("hideXAxis"):
            dt, x_label_delta = find_x_times(
                self.start_dt,
    ...
    def drawGridLines(self):
        ...
        for i, value in enumerate(labels):
            ...
            self.setColor(
                self.params.get(
                    "majorGridLineColor", self.defaultMajorGridLineColor
                )
            )
        ...
        for slice in self.slices:
            if self.valueLabels == "percent":
                ...
                if slice["value"] < self.valueLabelsMin:
                    continue
                if slice["value"] < 10 and slice["value"] != int(
                    slice["value"]
                ):
...
GraphTypes = {"line": LineGraph, "pie": PieGraph}
...
def dataLimits(data, drawNullAsZero=False, stacked=False):
    ...
    finiteData = [
        series for series in data if not series.options.get("drawAsInfinite")
    ]
