...
from .base import BaseTagDB, TaggedSeries
...
class RedisTagDB(BaseTagDB):
    ...
    def __init__(self, config, *args, **kwargs):
        super(RedisTagDB, self).__init__(config, *args, **kwargs)

        from redis import Redis

        self.directories = config['whisper']['directories']
        self.r = Redis(
            host=config['tagdb']['redis']['host'],
            port=config['tagdb']['redis']['port'],
            db=config['tagdb']['redis']['db'],
            password=config['tagdb']['redis']['password'],
