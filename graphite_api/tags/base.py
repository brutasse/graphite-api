...
class BaseTagDB(object):
    ...
    def find_series(self, tags, requestContext=None):
        ...
        try:
            ...
            if result is not None:
                ...
                if self.cache:
                    self.cache.set(cacheKey, result, 60)
        except Exception:
            log_msg = 'failed in'
            raise
        finally:
    ...
    def auto_complete_tags(self, exprs, tagPrefix=None, limit=None, requestContext=None):
        ...
        """
        if limit is None:
            limit = self.config.get('tagdb', {}).get('autocomplete_limit') or 100
        else:
            limit = int(limit)
    ...
    def log_info(self, func, msg):
        logger.info('%s.%s.%s :: %s' % (self.__module__, self.__class__.__name__, func, msg))
