...
aggregationTypeToMethod = dict(
    {1: "average", 2: "sum", 3: "last", 4: "max", 5: "min"}
)
aggregationMethodToType = dict(
    [[v, k] for k, v in aggregationTypeToMethod.items()]
)
...
def enableDebug():
    ...
    def endBlock(name):
        debug(
            "%s took %.5f seconds"
            % (name, time.time() - __timingBlocks.pop(name))
        )
    ...
    for i in xrange(archiveCount):
        ...
        try:
            ...
            raise CorruptWhisperFile(
                "Unable to read archive%d metadata" % i, fh.name
            )
...
"""
    ...
    try:
        ...
        try:
            newAggregationType = struct.pack(
                longFormat, aggregationMethodToType[aggregationMethod]
            )
        except KeyError:
        ...
        packedMetadata = (
            newAggregationType + maxRetention + xff + archiveCount
        )
        fh.seek(0)
...
"""
    ...
    try:
        ...
        aggregationType = struct.pack(
            longFormat, aggregationMethodToType.get(aggregationMethod, 1)
        )
        oldest = max(
            [
...
def __propagate(fh, header, timestamp, higher, lower):
    ...
    (higherBaseInterval, higherBaseValue) = struct.unpack(
    ...
    if knownPercent >= xff:  # we have enough data to propagate a value!
        ...
        myPackedPoint = struct.pack(
            pointFormat, lowerIntervalStart, aggregateValue
        )
...
def file_update_many(fh, points):
        ...
        while (
            ...
            if (
                ...
                __archive_update_many(
                    fh, header, currentArchive, currentPoints
                )
                currentPoints = []
            try:
                currentArchive = next(archives)
    ...
    if (
        currentArchive and currentPoints
...
def __archive_update_many(fh, header, archive, points):
    ...
    alignedPoints = dict(
        alignedPoints
    ).items()  # Take the last val of duplicates
...
"""
    fromInterval = (
        int(fromTime - (fromTime % archive["secondsPerPoint"]))
        + archive["secondsPerPoint"]
    )
    untilInterval = (
        int(untilTime - (untilTime % archive["secondsPerPoint"]))
        + archive["secondsPerPoint"]
    )
...
def file_merge(fh_from, fh_to):
    ...
    for archive in archives:
        ...
        (timeInfo, values) = __archive_fetch(
            fh_from, archive, fromTime, untilTime
        )
...
def file_diff(fh_from, fh_to, ignore_empty=False):
    ...
    for archive_number, archive in enumerate(archives):
        ...
        (toTimeInfo, toValues) = __archive_fetch(
            fh_to, archive, startTime, untilTime
        )
