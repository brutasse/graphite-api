...
default_conf = {
    ...
    "whisper": {"directories": ["/srv/graphite/whisper"]},
    ...
    def format(self, record):
        if not record.name.startswith("graphite_api"):
            ...
            return self._bound._process_event(
                record.levelname.lower(), record.getMessage(), kw
            )[0]
...
def configure(app):
    config_file = os.environ.get(
        "GRAPHITE_API_CONFIG", "/etc/graphite-api.yaml"
    )
...
def configure_logging(config):
    ...
    config["logging"]["handlers"].setdefault(
        "raw",
        {
            "level": "DEBUG",
    ...
    config["logging"]["loggers"].setdefault(
        "root", {"handlers": ["raw"], "level": "DEBUG", "propagate": False}
    )
    config["logging"]["loggers"].setdefault(
        "graphite_api", {"handlers": ["raw"], "level": "DEBUG"}
    )
